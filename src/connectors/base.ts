...
export type BaseConnectorOptions = InjectedConnectorOptions & {
  /**
...
export class BaseConnector extends InjectedConnector {
  protected shimDisconnectKey = `${this.id}.shimDisconnect`;

  #UNSTABLE_shimOnConnectSelectAccount: BaseConnectorOptions['UNSTABLE_shimOnConnectSelectAccount'];
...
export function getProviderFn(windowKey: string, getReadyKey: string) {
  return function getProvider() {
    function getReady(ethereum?: WindowProvider) {
      // const isClover = !!(ethereum as any)?.isClover;
      const isHave = !!ethereum && !!(ethereum as any)?.[getReadyKey];
      if (!isHave) return;
