...
export class BLOCTOConnector extends BaseConnector {
  readonly id = 'blocto';
  constructor({
    chains,
    options: options_,
    ...
    super({
      chains,
      options: {
        getProvider: getProviderFn('bloctoProvider', 'isBlocto'),
        name: 'Blocto',
      },
    });
  }
...
//   readonly ready =
//     typeof window != 'undefined' && !!this.#findProvider(window.bloctoProvider);

//   #provider?: Window['bloctoProvider'];
...
//       this.#provider = this.#findProvider(window.bloctoProvider);
//     }
...
//     if (!ethereum) return;
//     const isBlocto = !!(ethereum as any)?.isBlocto;
//     if (!isBlocto) return;
